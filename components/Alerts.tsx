...
const Alerts: React.FC<AlertsProps> = ({ bills, reminders }) => {
    ...
    return (
        <div className="bg-amber-50 border-l-4 border-accent p-4 rounded-r-lg shadow-md mb-8" role="alert">
            <div className="flex">
                <div className="py-1">
                    <svg className="fill-current h-6 w-6 text-accent mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-5.414V8a1 1 0 112 0v4.586a1 1 0 11-2 0zM10 6a1 1 0 100-2 1 1 0 000 2z"/></svg>
                </div>
                <div className="flex-grow">
                    <p className="font-bold text-amber-900">Upcoming: {upcomingItems.length} item(s) due within 7 days</p>
                    <ul className="list-disc list-inside text-sm mt-1 text-amber-800">
                ...
                <button onClick={() => setIsVisible(false)} className="ml-auto -mt-2 -mr-2 text-accent/70 hover:text-accent text-2xl font-bold">&times;</button>
            </div>
        </div>
    );
};
